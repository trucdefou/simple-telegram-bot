...
import logging
sys.path.append('..')
def load_enoch_reminder():
    try:
        with open("enoch_reminder.json", "r") as file:
            return json.load(file)
    except FileNotFoundError:
        return []

        
def save_enoch_reminder(list):
    with open("enoch_reminder.json", "w") as file:
...
async def handle_get_text(update: Update, context: CallbackContext):
    try:
        ...
        enoch_reminder = load_enoch_reminder()
        enoch_reminder.append(reminder_entry)  # Append the serialized reminder
        save_enoch_reminder(enoch_reminder)
...
async def enoch_reminder(update: Update, context: CallbackContext):
    enochreminder = load_enoch_reminder()
    message = "Recordatorios:\n"
    if enochreminder:
        for element in enochreminder:
            message += f"- {element}\n"
    else:
...
async def delete_from_reminder(update: Update, context: CallbackContext):
    ...
    buttons = [
        ...
        for idx, value in enumerate(enoch_reminder)
    ]
...
async def handle_delete_reminder(update: Update, context: CallbackContext):
    ...
    if data.startswith("delete_"):
        ...
        enochreminder = load_enoch_reminder()
        if item_to_delete < len(enochreminder):
            del enochreminder[item_to_delete]
            save_enoch_reminder(enochreminder)
...
async def daily_reminder(context: CallbackContext):
    try:
        ...
        save_enoch_reminder(updated_tasks)
